...
typedef struct list {
    ...
    char *endaddress;
    Node *lastprocessed;
    pthread_mutex_t mutex; /*Kilitleme mekanizması*/
    pthread_cond_t cond;   /*Koşul değişkeni*/
    int is_writing;        /* Yazma işleminin olup olmadığını takip etmek için*/
    sem_t sem_full;             /*capacity kontrolü için semaphore*/
    sem_t sem_empty;            /* element sayısı kontrolu için */
    /*ops on the list*/
