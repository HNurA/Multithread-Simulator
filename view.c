...
static void draw_rectangles() {
    ...
    }/* drone ların haritada çizilmesi için gerekli kod bloğu eklendi*/
    if(drones != NULL){
        Node *drone_node = drones->head;
        while(drone_node != NULL){
            Drone *drone = (Drone *)drone_node->data;
            SDL_Rect rect = {CSIZE * drone->coord.y, CSIZE * drone->coord.x, CSIZE, CSIZE};
