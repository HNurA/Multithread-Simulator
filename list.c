...
List *create_list(size_t datasize, int capacity) {
    ...
    pthread_mutex_init(&(list->mutex), NULL); /* Kilidi başlatma */
    pthread_cond_init(&(list->cond), NULL);   /* Koşul değişkenini başlatma*/
    list->is_writing = 0; /*Başlangıçta yazma işlemi yok*/

    sem_init(&list->sem_full, 0, capacity); /*capacity semaphore initi */
    sem_init(&list->sem_empty, 0, 0); /*semaphore initi listede 0 element var */
...
Node *add(List *list, void *data) {
    sem_wait(&list->sem_full); /*liste full ise bekle*/
    pthread_mutex_lock(&(list->mutex)); 

    while (list->is_writing) { /*Yazma işlemi varsa bekle*/
...
int removedata(List *list, void *data) {
    ...
    pthread_mutex_lock(&list->mutex); /*liste mutexini al*/
    ...
    if (temp != NULL) {
        ...
        temp->prev = NULL;
        temp->occupied = 0;
        sem_post(&list->sem_full); /*sem_full u bir azalt*/
        pthread_mutex_unlock(&list->mutex); 
        return 0;
    }
    pthread_mutex_unlock(&list->mutex);
...
void *pop(List *list, void *dest) {
    sem_wait(&list->sem_empty); /*liste boş ise bekle*/
    pthread_mutex_lock(&list->mutex);
    if (list->head != NULL) {
        Node *node = list->head;
...
void destroy(List *list) {
    ...
    memset(list, 0, sizeof(List));
    free(list);
    /*mutexleri ve semaphoreları destroy et*/
    pthread_mutex_destroy(&(list->mutex)); 
    pthread_cond_destroy(&(list->cond));   
    sem_destroy(&list->sem_full);
    sem_destroy(&list->sem_empty);
    free(list);
}
